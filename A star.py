...
class PathNode:
    ...
    def __lt__(self, other):
        # Compare based on the sum of cost, heuristic, and priority (A* search)
        return (self.cost + self.heuristic + self.priority) < (other.cost + other.heuristic + other.priority)
...
def a_star_search(graph, start, goal):
    ...
    while not priority_queue.empty():
        # Get the path with the lowest cost, heuristic, and priority from the queue
        ...
        for neighbor in graph[current_node]:
            ...
            # Create a new path node for the neighbor with priority
            ...
            new_path_node.priority = priority
...
optimal_path, optimal_cost = a_star_search(G, "P", "S")
...
plt.title("A* Search Algorithm Visualization (Optimal Path)")
plt.axis("off")
plt.show()
