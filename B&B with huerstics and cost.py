...
def calculate_heuristic(node, goal):
    # Define heuristic values based on your problem
    heuristic_values = {
        "P": 10,
        "R": 8,
        "A": 11,
...
def branch_and_bound(graph, start, goal, heuristic_func):
    # Initialize priority queue (min-heap) to store partial paths
    priority_queue = PriorityQueue()
    
    # Add the initial path to the queue with cost and heuristic
    initial_path_node = PathNode(start, [start], 0, heuristic_func(start, goal))
    priority_queue.put(initial_path_node)
    
    while not priority_queue.empty():
        # Get the path with the lowest estimated cost from the queue
        ...
        for neighbor in graph[current_node]:
            ...
            neighbor_heuristic = heuristic_func(neighbor, goal)
...
# Find the optimal path using Branch and Bound
optimal_path, optimal_cost = branch_and_bound(G, "P", "S", calculate_heuristic)
